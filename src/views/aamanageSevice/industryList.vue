...
<el-header class="header">
  <el-form :inline="true" :model="formInline" class="form">
    <el-form-item>
      <el-button type="primary" icon="el-icon-edit-outline" @click="addItem">新增行业</el-button>
    </el-form-item>
  </el-form>       
</el-header>
<el-main>
...
// getList 接口 获取
// addNotice 接口 添加
import {addIndustry_api,deleteIndustry_api,editIndustry_api,getIndustryList_api} from '@/api/admin' 
const formForNotive = {
        title:'',
        // num:''
      }
export default {
  ...
  methods: {
    ...
    deleteNewNotice(id){
      ...
      deleteIndustry_api(sendData).then(res=>{
        if(res&&res.status===0){
            this.$notify({
            title: '成功',
            message: '操作成功',
      ...
      getIndustryList_api(sendData).then(response => {
        if(response&&response.status==0){
          ...
          result.forEach((aData)=>{
            tempTableData.push({
              //后端生成
              id:aData.storeclass_id,
              //前后统一
              title:aData.storeclass_name,
            })
          })
