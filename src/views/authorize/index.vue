...
export default {
  ...
  data() {
    return {
      ...
      dialogConfig: {
        ...
        classList: [],
        addList: [
          { key: '姓名', value: 'seller_nick', isText: true, },
          { key: '账号', value: 'seller_name', isText: true },
          { key: '密码', value: 'seller_pwd', isText: true },
          { key: '发型师列表', value: 'stylist', isRadio: true, },
          { key: '订单列表', value: 'order', isRadio: true, },
          { key: '评论列表', value: 'comment', isRadio: true, },
          { key: '门店信息', value: 'storeInfo', isRadio: true },
      ...
      classList: [
        { key: '账号', value: 'seller_name', },
        { key: '发型师列表', value: 'stylist', isIcon: true },
        { key: '订单列表', value: 'order', isIcon: true },
        { key: '评论列表', value: 'comment', isIcon: true },
        { key: '门店信息', value: 'storeInfo', isIcon: true },
  ...
  methods: {
    ...
    submit(){
      let o = this.formData,
          param = {
            ...
            seller_nick: o.seller_nick,
            seller_password: o.seller_pwd,
    ...
    async getList(){
      ...
      if(res && res.data){
        res.data.forEach(v => {
          ...
          v.seller_limits.forEach(k => v[k] = 1);
        });
    ...
    async saveUser(param){
      let res = this.isAdd ? await api.setUser(param, this) : await api.updateUser(this.formData.seller_id, param, this);
    ...
    async deleteUser(item){
      let res = await api.deleteUser(item.seller_id, null, this);
