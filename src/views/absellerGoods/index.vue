...
<el-dialog
  ...
  <el-form :model="formForNotive" v-loading="editLoading" ref="ruleForm" :rules="rules" size="medium" >
    ...
    <el-form-item label="上级能拿到的积分" :label-width="formLabelWidth">
      <el-input v-model="rank_2" auto-complete="off"></el-input>
    </el-form-item>
        ...
        school: [
          {
            type: "array",
            required: true,
            message: "请输入门店，如果没有门店请先添加门店(运营=>门店)",
            trigger: "blur"
          }
        ],
        // goodsTotal: [
  ...
  methods: {
    ...
    formForNotiveIs_virtual_fn(nowValue) {
      ...
      if (nowValue) {
        ...
        if (getNames("names") && true)
          this.goodsTypehbsList.push(
            { value: 2, label: "课程" },
            { value: 3, label: "预约课程" }
          );
      } else {
    ...
    async addNewNotice(formName) {
      ...
      if (
        !(
          this.formForNotive.goodsType === 2 ||
          this.formForNotive.goodsType === 3
        )
      ) {
      ...
      if (
        (this.formForNotive.goodsType === 2 ||
          this.formForNotive.goodsType === 3) &&
        !(this.formForNotive.goodsPrice > 0)
      ) {
        return this.$message.error({ message: "值必须大于零" });
      }
      // 通过验证

      this.waitAddNotice = true;
      let sendData = {};
      sendData.rank_1 = this.rank_1;
      sendData.rank_2 = this.rank_2;
      sendData.rank_3 = this.rank_3;
      ...
      for (let i = 0, len = this.schoolList.length; i < len; i++) {
        ...
        if (this.formForNotive.school.indexOf(this.schoolList[i].value) >= 0) {
          tempArray.push({
            id: this.schoolList[i].value,
            name: this.schoolList[i].label
          });
        }
      ...
      if (
        ...
        sendData.is_appoint =
          this.formForNotive.goodsType === 1 ||
          this.formForNotive.goodsType === 3;
      }
      ...
      if (this.formForNotive.size === "one") {
        ...
        if (o.goodsType === 2 || o.goodsType === 3) {
          sendData.goods_price = sendData.goods_marketprice = sendData.goods_costprice = Number(
            o.goodsPrice
          );
          ...
          this.courseDetailList.forEach(v => {
            let val = `${v.teacher_name}-[${v.cschedule_id}]-${
              v.address_name
            }-${v.time_data ? v.time_data[0] : ""}`;

            tempMutil.push({
              price: sendData.goods_price,
              marketprice: sendData.goods_price,
              sp_value: val,
              stock: v.max_stunum
            });
    ...
    async editNewNotice(formName) {
      ...
      if (this.formForNotive.size === "one") {
        ...
        if (o.goodsType === 2 || o.goodsType === 3) {
          ...
          this.courseDetailList.forEach(v => {
            if (v.stock > v.max_stunum) {
              return (err = { message: "库存数量不能超过可预约人数" });
            }
      ...
      getGoods_api(sendData)
        .then(async data => {
          ...
          if (data.status === 0) {
            ...
            this.rank_1 = data.rank_1;
            this.rank_2 = data.rank_2;
            this.rank_3 = data.rank_3;
            ...
            tempForm.goodsType = data.is_course
              ? data.is_appoint ? 3 : 2
              : data.is_appoint;
            ...
            if (tempForm.size === "mutil") {
              ...
              if (tempForm.goodsType === 2 || tempForm.goodsType === 3) {
                ...
                this.courseDetailList.forEach(
                  (v, i) => (v.stock = data.SKUList[i].goods_storage)
                );
              } else {
    ...
    async downMutilItem(wantUp) {
      this.$confirm(
        `此操作将${wantUp === 1 ? "批量上架" : "批量下架"}该商品, 是否继续?`,
        "提示",
        {
