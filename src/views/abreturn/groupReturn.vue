...
<template>
  <div>
    <el-dialog title="商品明细" :visible.sync="detailShow" width="80%">
      <el-form :model="formForNotive" v-loading="editLoading" size="medium">
        <el-form-item label="编号" :label-width="formLabelWidth">
          <p class="hbs-no-margin-p">{{formForNotive.num}}</p>
        </el-form-item>
        <el-form-item label="金额" :label-width="formLabelWidth">
          <p class="hbs-no-margin-p">{{formForNotive.amount}}</p>
        </el-form-item>
        <el-form-item label="折扣" :label-width="formLabelWidth">
          <p class="hbs-no-margin-p">{{formForNotive.fee}}</p>
        </el-form-item>
        <el-form-item label="运费" :label-width="formLabelWidth">
          <p class="hbs-no-margin-p">{{formForNotive.trans}}</p>
        </el-form-item>
        <el-form-item label="下单时间" :label-width="formLabelWidth">
          <p class="hbs-no-margin-p">{{formForNotive.orderTime}}</p>
        </el-form-item>
        <el-form-item label="付款时间" :label-width="formLabelWidth">
          <p class="hbs-no-margin-p">{{formForNotive.payTime}}</p>
        </el-form-item>
        <el-form-item label="退款时间" :label-width="formLabelWidth">
          <p class="hbs-no-margin-p">{{formForNotive.returnTime}}</p>
        </el-form-item>
        <el-form-item label="订单状态" :label-width="formLabelWidth">
          <p class="hbs-no-margin-p">{{formForNotive.state}}</p>
        </el-form-item>
        <el-form-item label="订单列表" :label-width="formLabelWidth">
          ...
          >
            ...
            <el-table-column label="编号" prop="goods_num"></el-table-column>
            <el-table-column label="实付金额" prop="goods_pay_price"></el-table-column>
          </el-table>
        </el-form-item>
        <el-form-item label="收货信息" :label-width="formLabelWidth" v-if="formForNotive.buyerInfo">
          <el-form>
            <el-form-item label="收货地址" :label-width="formLabelWidth">
              <p class="hbs-no-margin-p">{{formForNotive.buyerInfo.address}}</p>
            </el-form-item>
            <el-form-item label="收货人" :label-width="formLabelWidth">
              <p class="hbs-no-margin-p">{{formForNotive.buyerInfo.name}}</p>
            </el-form-item>
            <el-form-item label="收货人电话" :label-width="formLabelWidth">
              <p class="hbs-no-margin-p">{{formForNotive.buyerInfo.phone}}</p>
            </el-form-item>
          </el-form>
        </el-form-item>
      </el-form>
      ...
      <el-main>
        ...
        >
          <el-table-column label="商品图片">
            <template slot-scope="scope">
              <div style="width:100px;height:100px;align-items:center;display:flex;">
                <img :src="scope.row.goods_image" alt style="width:100px">
              </div>
            </template>
          </el-table-column>
          <el-table-column label="商品名字" prop="goods_name"></el-table-column>
          <el-table-column label="价格" prop="total_fee"></el-table-column>
          <el-table-column label="订单编号" prop="order_sn"></el-table-column>
          <el-table-column label="买者姓名" prop="member_truename"></el-table-column>
          <el-table-column label="买家电话" prop="member_mobile"></el-table-column>
          <el-table-column label="支付时间" prop="payment_time"></el-table-column>
          <el-table-column label="支付金额" prop="refund_fee"></el-table-column>
          ...
          <el-table-column label="操作">
            <template slot-scope="scope">
              <el-button size="mini" type="info" @click="deleteItem(scope.$index, scope.row)">删除</el-button>
              <el-button
                size="mini"
                type="danger"
                v-if="scope.row.refund_state==='CHECK'"
...
export default {
  ...
  methods: {
    ...
    changeNewNotice(id, num) {
      ...
      editGroupReturn(sendData)
        ...
        .catch(err => {
          ...
          console.error("editGroupReturn:", err);
        });
    },
    deleteGroupReturn(id, num) {
      let sendData = {
        refund_id: id,
      };
      deleteGroupReturn(sendData)
        .then(res => {
          if (res && res.status === 0) {
            this.$notify({
              title: "成功",
        ...
        .catch(err => {
          this.waitAddNotice = false;
          console.error("deleteGroupReturn:", err);
        });
    },
    
    changeItem(index, raw) {
      let id = raw.id;
      this.$confirm(`请确认退款`, "提示", {
    ...
    async getList(all) {
      ...
      await groupReturn_api(sendData).then(response => {
        ...
        if (response && response.status == 0) {
          ...
          result.forEach(aData => {
            tempTableData.push({
              id: aData.refund_id,
              goods_image: aData.goods_image,
              goods_name: aData.goods_name,
              total_fee: aData.total_fee,
              order_sn: aData.order_sn,
              member_truename: aData.member_truename,
              member_mobile: aData.member_mobile,
              payment_time: aData.payment_time,
              refund_fee: aData.refund_fee,
              refund_state: aData.refund_state,
              data: aData
