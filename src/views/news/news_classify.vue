...
<el-main>
    <el-table :data="tableData" stripe v-loading="listLoading" element-loading-text="给我一点时间" style="width: 100%" >
      <el-table-column label="分类名字" prop="consult_classname"></el-table-column>
...
import { getNewsClassify_api, deleteNewsClassify_api, addNewsClassify_api, editFlowPackage_api } from '@/api/admin' 
...
export default {
  ...
  methods: {
    ...
    async addAuth(formName) {
      ...
      let sendData = {
        consult_classname: this.formForNotive.value,
      }

      addNewsClassify_api(sendData).then(data=>{
        console.log(data);
    ...
    // body
      getList() { //获取列表
        ...
        getNewsClassify_api(sendData).then(response => {
      ...
      deleteItem(index, row) {
        ...
        this.$confirm(`此操作将删除该条目, 是否继续?`, '提示', {
          ...
          this.deleteNewNotice(row.consult_classid) 
        }).catch(()=>{
      ...
      deleteNewNotice(id) {
        ...
        deleteNewsClassify_api(sendData).then(res=>{
          if(res&&res.status===0){
            this.$notify({ title: '成功', message: '操作成功', type:'success' });
            this.getList()
          }else{
