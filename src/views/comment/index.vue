...
<el-container class="notice">
  ...
  <el-header class="header">
    ...
    <custom-head :config='headConfig' @add='updateForm' @searchByDate='getKeyword' @searchByStatus='getStatus'></custom-head> 
...
export default {
  ...
  methods: {
    getKeyword(v){
      v ? this.param.search = v : delete this.param.search;
      this.getList();
    },
    getStatus(v){
      console.error('comment', v);
      if(v != 2) this.param.isanonymous = v;
      this.getList();
    },
    async initForm(item){
      let o = {};
      ...
      this.dialogConfig.classList[3].list = [ {store_id: item.geval_storeid, store_name: item.geval_storename } ]; // store_res;
