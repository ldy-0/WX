<template>
  <div :width="'100%'">
    ...
    <el-form :label-width="config.labelWidth || '100px'" :disabled='config.isDisabled' :model='detail' ref='ruleForm' :rules='config.rules' v-else>
      ...
      <el-form-item :label="item.key"  
        ...
        <!--  Time 
              dateType: | timerange
         -->
        <el-time-picker style="width:400px" range-separator="至" start-placeholder="开始日期" end-placeholder="结束日期" 
        ...
        <div v-if='item.isAddress'>
          ...
          <geocoder :address='detail[item.value]' @get='getLocation'></geocoder>
        </div>
    ...
    <span slot="footer" class="dialog-footer" :class="{ 'footer_center': config.isCenter }" v-if='!config.isDisabled'>
      <el-button @click="cancel" v-if='!config.showCancel'>取消</el-button>
      <el-button :class="{ 'center': config.isCenter }" type="primary" :disabled="!config.canSubmit" :loading="!config.canSubmit" @click="submitForm('ruleForm')">确 定</el-button>
    </span>
...
export default {
  ...
  methods: {
    ...
    input(item){
      ...
      if(item.isInteger && v % 1 !== 0)return this.error.push({ prop: item.value, message: '值必须为整数', duration: 1200 });
