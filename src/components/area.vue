  ...
  <view v-if="show" class="cascade_box">
    ...
    <view class="cascade_find" :animation="animationData.showAnim">
      ...
      <picker-view :value="defaultValue" indicator-style="height: 80rpx;font-size: 38rpx" style="width: 100%; height: 400rpx;" @change="bindChange">
        ...
        <picker-view-column class="pvc">
          <view v-for="(item, index) in cities" :key='index' style="line-height: 80rpx;text-align:center;">{{item.name}}</view>
        </picker-view-column>
        <picker-view-column class="pvc">
          <view v-for="(item, index) in areas" :key='index' style="line-height: 80rpx;text-align:center;">{{item.name}}</view>
        </picker-view-column>
      </picker-view>
    </view>
  </view>
