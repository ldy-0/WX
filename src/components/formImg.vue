      ...
      <view class='imgs_wrap flex'>
        <repeat for='{{obj.value}}'>
          <view class='img_wrap'>
            ...
            <image class='i_close' src='{{closeURL}}' @tap="closeImg({{index}})" />
          </view>
        </repeat>

        <view class='add_img_wrap' @tap="addImg" wx:if='{{canAdd}}'>
          <image class='add_img' src='{{addImgURL}}' />
          <view class='add_desc s_fc_10'>添加图片</view>
        </view>
      </view>
...
export default class customInput extends wepy.component {
  ...
  computed = {
    canAdd(){ return this.obj.value ? this.obj.value.length < (this.obj.limit || 9) : true; },
  ...
  methods = {
    async addImg(){
      ...
      if(res){
        ...
        o.value = o.value.concat(len < imgs.length ? imgs.slice(0, len) : imgs);
      }
