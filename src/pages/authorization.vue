...
<template>
  <view class="container s-bg-2">
    ...
    <view class='btn flex' wx:if='{{getinfoBtn}}'>
      <image class='i_weixin' src='{{weixinURL}}' />
      <button class="clear s-fc-2" open-type="getUserInfo" lang="zh_CN" bindgetuserinfo="onGotUserInfo" plain='true'>微信登入</button>
    </view>
    <button wx:if='{{!getinfoBtn}}' class="btn s-fc-2" @tap="onGotPhone">微信登入</button>
...
export default class Authorization extends wepy.page {
  config = {
    ...
    navigationBarBackgroundColor: "#10325f",
    navigationBarTextStyle: "#fff"
  };

  data = {
    weixinURL: '../images/weixin.png',
    path: null,
    shareType: null,
    getPhoneShow: false,
  ...
  onLoad(options) {
    // return wx.switchTab({ url: '/pages/home' });
