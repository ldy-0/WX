    ...
    <repeat for="{{orderDetail.order_goods}}" key="index" index="index" item="item">
      <view class="product_info around">
        ...
        <view class="product">
          ...
          <view class="row">
            <view class="product_standard">{{item.goods_spec|| '统一规格'}}</view>
    ...
    <repeat for="{{list}}" item="item">
      <view @tap="gotoReturn({{item.url}})" class="row_between">
        <view class="flex">
          ...
          <text class='desc s_fc_2'>{{item.info}}</text>
        </view>
...
export default class Select extends wepy.page {
  ...
  data = {
    ...
    list: [
      {
        name: "仅退款",
        url: "/pages/order/refund?type=refund&goods=",
        info: "未收到货（包含未签收)"
      },
      {
        name: "退货退款",
        url: "/pages/order/refund?type=returnGoods&goods=",
  ...
  methods = {
    ...
    gotoReturn(url) {
      wx.navigateTo({
        url: url + encodeURIComponent(JSON.stringify(this.orderDetail))
      });
    }
  };
  onLoad(option) {
