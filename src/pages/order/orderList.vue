...
<template>
  <view class="container">
    ...
    <view class="selection">
        <view class="tab">
            <view class="{{item.id === checkId ? 'active' : 'unactive'}}" wx:for="{{choose}}" wx:key @tap="changeIndex({{item}})">{{item.name}}</view>
        </view>
    </view>
        ...
        <repeat for="{{orderList}}" key="index" item="order">
          ...
          <view @tap="goOrderDetail" data-index="{{index}}">
            <repeat for="{{order.order_goods}}" item="items">
              <view class="product_info s_bg_2">
          ...
          <view class="operate_info">
            ...
            <view
              class="operate_infoBtn operate_infoBtnCol"
              wx:if="{{checkId != 10 && (order.order_state_id == 20 || order.order_state_id == 30 || order.order_state_id == 40) }}"
            ...
            <view
              class="operate_infoBtn"
              wx:if="{{order.order_state_id==80&&order.evaluation_state==1}}"
...
export default class OrderList extends wepy.page {
  ...
  onShow() {
    ...
    // 判断是否为从订单详情返回
    if(!this.query.hasOwnProperty('order_type')) this.query.order_type = 1;
  ...
  methods = {
    ...
    goOrderDetail(e) {
      ...
      wx.navigateTo({ url: `/pages/store/orderdetail?orderId=${this.orderList[index].order_id}&unVip=${this.query.order_type != 10}` });
    },
    changeIndex(item){
      this.query.order_type = this.checkId = item.id;
  ...
  async getList(){
    ...
    if (res.status === 0) {
      ...
      if (res.data != null && res.data.length != 0) {
        ...
        this.total = res.pagination ? res.pagination.total : this.orderList.length;
      }
    } else {
  ...
  //上拉加载
  onReachBottom() {
    if(this.total <= (this.page * this.query.limit)) return console.error('end');
  ...
  async salesReturn(e) {
    ...
    const res = await shttp.get("/api/v2/member/order/" + order.order_id).end();

    if(res && res.data){
      order.pd_amount = res.data[0].pd_amount;
    ...
    console.error(encodeURIComponent(JSON.stringify(order)));
    if (type == "1") {
      wx.navigateTo({
        url:
          "/pages/order/afterSale?goods=" + encodeURIComponent(JSON.stringify(order))
      });
    } else {
      wx.navigateTo({
        url:
