    ...
    <view wx:if="{{orderList.length != 0}}">
      <view class="waitView">
        <repeat for="{{orderList}}" index="index" item="item">
          <view class="title">
            {{item.order_state}}
            <image wx:if="{{item.order_state === '已完成'}}" class="i_done" src="../../images/img_6@2x.png"  />
            <image wx:if="{{item.order_state === '已完成'}}" class='i_del' src='../../images/order/del.png' @tap='delOrder({{item}})' />
          </view>
          ...
          <view class="waitBody">
            ...
            <view class="price_info s_fc_2">
              共{{item.order_goods.length}}件商品 实付款：¥<text class='price'>{{item.refund_amount}}</text>
              <!-- <view>实付款</view>
              <view>
            ...
            <!-- <view class='coupon_info_wrap s_fc_2 s_bg_1' wx:if="{{item.couponPrice > 0}}">优惠券：-¥{{item.couponPrice}}</view> -->
            <view class='coupon_info_wrap s_fc_2 s_bg_1' wx:if="{{item.refund_points > 0}}">退还积分 -¥{{item.refund_points}}</view>
            <view class='coupon_info_wrap s_fc_2 s_bg_1' wx:if="{{item.refund_pd_amount > 0}}">退还余额 -¥{{item.refund_pd_amount}}</view>

            <view class="operate_info">
              <button class="Customer operate_btn" open-type="contact" session-from="weapp" plain="true">联系客服</button>
              <view class='operate_btn cancel_btn s_fc_3' wx:if="{{item.order_state=='待退款'||item.order_state=='待退货'}}" @tap="cancelReturn({{item.refund_id}})">取消退款</view>
            </view>
          </view>
...
export default class Aftersale extends wepy.page {
  ...
  methods = {
    ...
    toDetail(item) {
      console.log(item);
      wx.navigateTo({
        url: "./aftersaleDetail?refund_id=" + item.refund_id
      });
    },
    async delOrder(item){
      let param = {
      ...
      wx.showModal({
        title: "提示",
        content: "是否删除订单",
        success: res => { if(res.confirm){ this.hideOrder(param) } },
      });
    }
  };
  //取消退款退货
  ...
  async getOrderList() {
    ...
    if (res.status == 0) {
      ...
      if (res.data) {
        res.data.forEach(v => {
          v.couponPrice = calc.sub(calc.add(v.goods_total_prices, v.shipping_fee), v.order_amount).toFixed(2);
  ...
  //改变订单状态
  async hideOrder(send){
    const res = await shttp.put(`/api/v2/member/refundreturn/${send.refund_id}`).send(send).end();
