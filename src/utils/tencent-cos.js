...
let config = {
  bucket: 'collie',
  appid: '1253114089',
  sid: 'AKIDSnxwjKoWs20mMmTvUs43pTvdEesu7QuG',
  skey: 'INGzWJyRimjWGpaGk1A6qNiroY7pNjKl',
  region: 'sh',
  currentSig: null,
  expireTime: null
};
...
let getUrl = (filePath) => {
  return 'https://' + config.region + '.file.myqcloud.com/files/v2/' + config.appid +
    '/' + config.bucket + '/' + filePath + '?sign=' + getAppSign();
};

let getAppSign = () => { // 获取签名 必填参数
...
let uploadFile = async (filePath, options) => {
  try {
    ...
    if (devtools === "true") {
      ...
      localFileName = `${nameArr[nameArrLength - 2]}.${nameArr[nameArrLength - 1]}`;
    }
    ...
    const sourceUrl = parseRes.data.source_url;
    // 腾讯云里面可以配置!compress的样式
    if (IMG_REG.test(sourceUrl)) {
      return sourceUrl.replace(/cossh/, "picsh").replace(/$/, "!compress");
    }
    return sourceUrl;
  } catch (error) {
    console.error(error);
