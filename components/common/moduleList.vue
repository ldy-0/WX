	...
	<view>
		<view class="list_wrap" :style="{ 
			...
			<view class="list_title_wrap between" v-if="config.title">
				<view class="list_title s_fc_3">{{config.title}}</view>
			...
			<view class="row between" v-for="(row, rowIndex) in config.list || list" :key="rowIndex">
				<view class="item" v-for="(item, index) in row" :key="index">
					<slot :item="item" :index="index" :rowIndex="rowIndex"></slot>
				</view>
			</view>
