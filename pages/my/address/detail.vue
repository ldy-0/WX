	...
	<view class="container page_wrap s_fc_12 s_bg_f">
		<form @submit="submit">
			<view class="row-flex">
				...
				<input type="text" name="name" class="inline" @input="setConsignee" :value="address.consignee" placeholder="请填写真实姓名" placeholder-class="placeholder">
			</view>

			<view class="row-flex">
				<text class="info-text">联系电话</text>
				<input type="number" name="phone" class="inline" @input="setPhone" :value="address.phone" placeholder="请填写收货人手机号" placeholder-class="placeholder">
			</view>
			...
			<view class="row-flex">
				...
				<input type="text" name="detail" class="inline" @input="setStreet" :value="address.street" placeholder="街道、楼牌号等" placeholder-class="placeholder">
			</view>
    ...
    components: {
      ...
      changeDefault() {
        ...
        [this.province, this.city, this.area] = arr;
			...
			setConsignee(e){ this.address.consignee = e.detail.value },
			setPhone(e){ this.address.phone = e.detail.value },
			setStreet(e){ this.address.street = e.detail.value },
        ...
        uni.showLoading({ title:'Loading', mask:true })
				...
				const res = await userApi.addAddress(param);
        uni.hideLoading();
        if(!res || typeof res === 'string' || res.error) return uni.showToast({ title: res ? res.error || res : '保存地址失败!', icon: 'none', }), this.isClick = true;
