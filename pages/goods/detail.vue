<template>
	<view class="goods_page_wrap page_wrap s_bg_f s_fc_gray">
    <view class="banner_wrap">
      ...
      <view class="goods_title mt20 s_fc_10">{{detail.goods_name}}</view>
			...
			<view class="flex s_fc_gray">
			  <view class="">{{detail.spec_name ? detail.spec_name[0] : ''}}</view>
			  <view class="ml48">销量：{{detail.goods_salenum}}</view>
			  <view class="ml48">库存：{{detail.goods_storage}}</view>
		  </view>
		</view>
    ...
    <view class="goods_content_wrap">
      <rich-text :nodes="seckill ? seckill.rule_body : detail.goods_body"></rich-text>
    </view>

		<view class="bottom_bar between s_bg_f">
			<view class="ml40 flex">
				<view class="">
					<image class="i_contact" src="../../static/goods/contact.png"></image>
					<view class="contact fs20">客服</view>
				</view>

				<view class="ml60" v-if="!isSeckill" @tap="goCart">
					<view class="cart_img_wrap">
						<image class="i_cart" src="../../static/goods/cart.png"></image>
			...
			<view class="seckill_btn flex s_bg_11 s_fc_f" @tap="goSubmit" v-if="isSeckill">
				...
				<uni-countdown class="seckill_goods_countdown fs24" :day="countdown.day" :hour="countdown.hour" :minute="countdown.minute" :second="countdown.second" color="#fff" splitor-color="#fff" background-color="transparent" />
				<view class="seckill_btn_title ml30">立即秒杀</view>
			</view>
...
export default {
	...
	computed: {
		isSeckill(){ return this.ruleId; },
		isStore(){ return this.user && this.user.member_rank == gd.DRUGSTORE; },
	},

	onLoad(opt) {
		this.id = opt.id;
		this.ruleId = opt.ruleId;
		this.user = uni.getStorageSync('user');
		if(!this.user) return uni.navigateTo({ url: `/pages/login/login`, });

		// 秒杀
		if(this.ruleId){
			return this.getSeckillDetail();
		}
	...
	methods: {
		preview(item) {
			uni.previewImage({ urls: this.bannerConfig.list.map(v => v.goodsimage_url || v) });
		},
		...
		goComment(e){
			...
			uni.navigateTo({ url: `/pages/goods/commentList?id=${this.detail.goods_commonid}`, });
		},
		...
		goSubmit(e){
			...
			if(this.detail.goods_storage <= 0) return uni.showToast({ title: '库存不足!', icon: 'none', });

			uni.showLoading({ title: '...', mask: true, });

			uni.setStorageSync('sku', this.detail);
		...
		async getCommentList() {
			...
			let res = await goodsApi.getCommentList({ goods_commonid: this.detail.goods_commonid, });
			uni.hideLoading();
		...
		async getSeckillDetail() {
			uni.showLoading({ title: 'loading...', mask: true, });

			let res = await goodsApi.getSeckill(this.ruleId);
			uni.hideLoading();
			if(!res || typeof res === 'string' || res.error) return uni.showToast({ title: res ? res.error || res : '!', icon: 'none', });
		
			if(res.data) {
				this.seckill = this.formatSeckill(res.data);
				this.getCommentList();
			}
		},
		...
		formatSeckill(v) {
			try{
				this.bannerConfig.list = JSON.parse(v.rule_images);
			}catch(e){
				console.log(e.message);
			}

			v.rule_body = v.rule_body.replace(/(<img )/g, '$1style="width: 100%;" ');
			
			this.detail = {
				goods_commonid: v.goods.goods_commonid,
				goods_price: v.goods_price,
				goods_marketprice: v.goods.goods_price, 
				goods_name: v.rule_name,
				spec_name: [v.goods.goods_spec],
				goods_salenum: mathjs.subtract(v.limit_num, v.goods.goods_storage),
				goods_storage: v.goods.goods_storage,

				goods_image: v.rule_image,
				seckill_id: v.rule_id,
				goods_id: v.goods.goods_id,
				goods_spec: v.goods.goods_spec,
				goods_num: 1,
			};

			let t = new Date(v.end_time.replace(/-/g, '/')) - Date.now();
			this.countdown = {
